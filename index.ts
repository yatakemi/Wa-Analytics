...
program
  .option(
    '--repo <owner/repo>',
    '分析対象のリポジトリ (例: octocat/Spoon-Knife)'
  )
  ...
  .option(
    '--time-unit <unit>',
    '時系列グラフの時間単位 (daily, weekly, monthly)',
    'daily'
  )
...
async function main() {
  ...
  if (options.analyzeAi) {
    if (!aiApiKey) {
      console.error(
        'エラー: 生成AIによる分析には環境変数 GEMINI_API_KEY または OPENAI_API_KEY が必要です。'
      )
      process.exit(1)
    }
  ...
  let startDate: Date | null = options.startDate
    ? startOfDay(parseISO(options.startDate))
    : null
  let endDate: Date | null = options.endDate
    ? endOfDay(parseISO(options.endDate))
    : null

  if (startDate && !isValid(startDate)) {
    console.error(
      'エラー: 無効な開始日フォーマットです。YYYY-MM-DD形式で指定してください。'
    )
    process.exit(1)
  }
  if (endDate && !isValid(endDate)) {
    console.error(
      'エラー: 無効な終了日フォーマットです。YYYY-MM-DD形式で指定してください。'
    )
    process.exit(1)
  }
  ...
  if (options.repo) {
    ...
    if (!owner || !repo) {
      console.error(
        'エラー: リポジトリの指定が不正です。owner/repo 形式で指定してください。'
      )
      process.exit(1)
    }
    ...
    if (startDate && endDate) {
      ...
      const pulls = await githubClient.getPullRequests(
        owner,
        repo,
        startDate,
        endDate
      ...
      if (prMetrics.mergedPullRequests > 0) {
        await reporter.generateChart(
          {
            labels: ['マージされたPR数'],
            values: [prMetrics.mergedPullRequests],
          },
          'merged_pr_count.png',
          'マージされたPull Request数',
          '数'
        ...
        await reporter.generateChart(
          {
            labels: ['平均PRサイクルタイム'],
            values: [prMetrics.avgTimeToMerge],
          },
      ...
      switch (timeUnit) {
        case 'daily':
          ...
          issueAvgResolutionTimeSeriesData =
            issueTimeSeries.daily.avgIssueResolutionTime
          break
        case 'weekly':
        ...
        case 'monthly':
          ...
          issueAvgResolutionTimeSeriesData =
            issueTimeSeries.monthly.avgIssueResolutionTime
          break
        default:
          console.warn(
            '警告: 無効な時間単位が指定されました。日次データを使用します。'
          )
          prMergedTimeSeriesData = prTimeSeries.daily.mergedPullRequests
          prAvgTimeToMergeTimeSeriesData = prTimeSeries.daily.avgTimeToMerge
          issueClosedTimeSeriesData = issueTimeSeries.daily.closedIssues
      ...
      console.warn(
        '警告: 分析期間が指定されていないため、データ収集とメトリクス計算はスキップされます。'
      )
    }
